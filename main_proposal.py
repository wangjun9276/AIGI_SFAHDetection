...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (inputs, labels) in enumerate(dl_train):
            ...
            outputs, pred_aux0, pred_aux1, pred_aux2, pred_aux3 = model(inputs)
            outputs = outputs.squeeze(1)
            loss = nn.BCEWithLogitsLoss()(outputs, labels)
            loss_aux0 = nn.BCEWithLogitsLoss()(pred_aux0.squeeze(1), labels)
            # loss_aux1 = nn.BCEWithLogitsLoss()(pred_aux1.squeeze(1), 1-labels)
            loss_aux1 = nn.BCEWithLogitsLoss()(pred_aux1.squeeze(1), labels)
            loss_aux2 = nn.BCEWithLogitsLoss()(pred_aux2.squeeze(1), labels)
            # loss_aux3 = nn.BCEWithLogitsLoss()(pred_aux3.squeeze(1), 1-labels)
            loss_aux3 = nn.BCEWithLogitsLoss()(pred_aux3.squeeze(1), labels)
            # train from 0.5 ( before 7-22-13:00), 0.1 (try later), average balance 0.2 (after 7-22-13:00)
            loss = loss + 0.1 * (loss_aux0 + loss_aux1 + loss_aux2 + loss_aux3)
            # loss = 0.2 * loss + 0.2 * (loss_aux0 + loss_aux1 + loss_aux2 + loss_aux3)
            # loss = (loss + loss_distillation * 0.5) / accumulation_steps
