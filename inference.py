...
def calculate_fps(
    ...
    fps = repeat * input_size[0] / total_time
    latency = total_time / repeat * 1000
...
datasets = {
    ...
    "ARForensics": {
        "test_root": os.path.join('/media/sda/junwang/datasets/ARForensics', "test"),
        "vals": ['Infinity','Janus_Pro','LlamaGen','MAR','Open_MAGVIT2','RAR','VAR']
    },
    "WildRF": {
        "test_root": os.path.join('/media/sda/junwang/datasets/WildRF', "test"),
        "vals": ['facebook', 'reddit',  'twitter']
    },
    # "genimage": {
    #     "test_root": os.path.join(BASE_DIR, "test"),
    #     "vals": [
...
class ForenSynths(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # image = jpeg_compression_in_buffer(image, 100)
            # image = pil_jpg(image, 50)
            # image = skimage.util.random_noise(np.array(image), mode='gaussian', var=0.0005, clip=True)
            # image = Image.fromarray(np.uint8(image*255.0))
            # image = add_gasuss_blur(np.array(image), kernel_size=(3, 3), sigma=0.7)
...
def validate(model, data_loader):
    ...
    with torch.no_grad():
        for img, label in data_loader:
            ...
            # outputs, outputs0, outputs1, outputs2, outputs3 = model(in_tens)
...
def test():
    ...
    # # input_ids = torch.zeros(1, 1, device=device)
    ...
    # print("Vision FLOPs:", flops.total() / 1e9, "GFLOPs")
    # print("Vision Params:", total_params / 1e6, "M")
