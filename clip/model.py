...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, MoE_args=0):
        ...
        # self.mask = nn.Parameter(On_attention_gaussian_mask((input_resolution // patch_size) ** 2), requires_grad=False)
        # # if self.is_mask:
        # #     self.pos_embedding = nn.Parameter(torch.randn(1, (input_resolution // patch_size) ** 2, self.dim))
        # # else:
        # #     self.pos_embedding = nn.Parameter(torch.randn(1, self.num_patches + 1, self.dim))
        # #     self.cls_token = nn.Parameter(torch.randn(1, 1, self.dim))

        # # if is_GMM:
        # self.mask = Gaussian_Mixture_Mask(num_heads=self.heads, num_kernals=5, mask=mask)
        # elif is_SLM:
        ...
        if self.MoE_args:
            self.transformer = MoETransformer(
                width, layers, heads, MoE_args=MoE_args)
        else:
            self.transformer = Transformer(width, layers, heads)
...
def build_model(state_dict: dict, MoE_args=None):
