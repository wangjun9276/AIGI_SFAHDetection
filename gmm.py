...
class Gaussian_Mixture_Mask(nn.Module):
    ...
    def forward(self, x):
        ...
        # exp = 1 / (self.sigma.clamp(min=1e-2) ** 2 + 1e-5)
        # exp = exp.clamp(max=30.0)  # 限制最大指数，避免数值爆炸
        # attn = torch.sum(self.alpha * self.mask ** exp, dim=0)

        # weight = torch.exp(-self.mask / (self.sigma.clamp(min=1e-2) ** 2 + 1e-5))
        # attn = torch.sum(self.alpha * weight, dim=0)
